...
object CryptoTrader {
  ...
  val placeOrderPath = "/v1/order/new"
  ...
  def main(args: Array[String]): Unit = {
    ...
    println("Buy/Sell?")
    val sideIn = scanner.next().toLowerCase
    val side   = Side.withName(sideIn(0).toUpper.toString ++ sideIn.substring(1))
    println("symbol")
    val symbol = CryptoSymbol.toCryptoSymbol(scanner.next().toLowerCase)
    if(symbol.isEmpty){
      println("Symbol not found exiting.")
      System.exit(1)
    }
    println("amount?")
    val amount          = new BigDecimal(scanner.next)
    println("price?")
    val price           = new BigDecimal(scanner.next)
    val totalBeforeFees = amount.multiply(price)
    val makerFee        = totalBeforeFees.multiply(makerFeeRate)
    val takerFee        = totalBeforeFees.multiply(takerFeeRate)

    def form(d: BigDecimal) = d.formatted("%.2f")
    println(
      s"""
         |$side $amount ${symbol.get.toString} for $price
         |totalBeforeFees: ${form(totalBeforeFees)}
         |makerFee: ${form(makerFee)}
         |takerFee: ${form(takerFee)}
         |makerCost: ${form(totalBeforeFees.add(makerFee))}
         |takerCost: ${form(totalBeforeFees.add(takerFee))}
         |""".stripMargin
    )
  ...
  def placeOrder(side: Side.Side,
    ...
    val urlPath = new URL(environment.url + placeOrderPath)
