...
object CryptoCancelAll {
  ...
  def main(args: Array[String]): Unit = {
    ...
    confirmation.toLowerCase match {
      case "y" =>
        cancelAll(environment)
      case "_" => println("Exiting")

    }
  }
  def cancelAll(environment: Environment) = {
    ...
    val urlPath = new URL(environment.url + cancelAllPath)
